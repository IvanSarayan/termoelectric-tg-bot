...
function showAnswer (msg, key) {
    if(key === 1) {
        bot.sendMessage(msg.chat.id, `Радиатор представляет собой алюминиевый радиатор с запрессованным в каждую секцию теплоемким сердечником и интегрированным в него нагревательным элементом (собственная разработка компании “TermoElectric”):\n -нагревательный элемент - углеродный композит\n -повышенной тепловой мощности\n -работает 15-20 минут в час\n -без каких-либо жидкостей`)
    }
    if(key === 2) {
